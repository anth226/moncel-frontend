    ...
    return <div id={`nav-menu-${ID}`} onMouseLeave={closeMenu}>
        ...
        <Popper id={ID} open={props.open === ID} anchorEl={props.anchor} placement="bottom-start">
            ...
            <div className="padded-section bg-white px-7 py-8 grid grid-cols-2 gap-x-8 gap-y-0 grid-flow-row mt-3">
                <Column>
                    <Header4 className="text-mine leading-6">Resource Library</Header4>
                    <Divider />
                    <div style={{
                        display: "grid",
...
const Link = ({ text, href }: { text: string, href: string }) => <a className="link text-mine no-underline hover:underline font-medium" href={href} target="_blank" rel="noreferrer">{text}</a>
