...
const TwoColSection = (props: TwoColStoryblok & StoryblokStoryProps) => {
    ...
    const imgComp = props.story ? <DynamicImage fileNode={fileNode} alt={`${props.title} image`} className="rounded-lg" /> : <img src={props.image?.filename} className="rounded-lg" alt={`${props.title} image`} />;
    return <div>
