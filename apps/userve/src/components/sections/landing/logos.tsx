    ...
    return <Section>
            ...
            <div className="grid grid-cols-12 grid-flow-row">
                { logos.filter((logo, i) => logo != undefined).map((logo, i) => {
                    ...
                    return <div className="col-span-6 md:col-span-2 row-span-1 h-28 items-center justify-center flex" key={`logos-${i}`}><DynamicImage fileNode={localImageFileNode} className="block m-auto max-w-100 max-h-[6rem]" alt={`${logo?.alt || "Userve"}`}/></div>
                })}
            </div>
        </Section>
