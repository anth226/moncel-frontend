        ...
        <Section>
            ...
            <div className="grid grid-cols-3 grid-flow-row gap-3">
                { logos.filter((logo, i) => logo != undefined).map((logo, i) => {
                    return <div className="col-span-1 row-span-1 h-28 items-center justify-center border-2 border-solid border-indigo-200 rounded-lg p-4" key={`logos-${i}`}><img src={logo!.filename || ""} className="h-full block m-auto" /></div>
                })}
            </div>
        </Section>
    </SectionFullWidth>
