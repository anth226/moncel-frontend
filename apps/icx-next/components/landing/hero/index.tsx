...
const Hero = ({ hero }: { hero: ComponentLandingHeroSection  }) => {
    ...
    return <div className="container">
        ...
        <div className="col-12 col-md-7 col-lg-6 py-0 py-md-8 text-center text-md-start">
            <ReactMarkdown className="text-white text-3xl pb-4">{hero.title}</ReactMarkdown>
            <Link href={hero.Button.href} passHref><div className="btn btn-primary text-white">{hero.Button.text}</div></Link>
        </div>
        { isTablet ? null : <div className={'col-12 col-md-5 col-lg-4 offset-lg-2 d-none d-md-block'}>
            <div style={{
                width: imageDimensions[0],
                height: imageDimensions[1],
            }}><Image src={`${strapiURL}${hero.HeroImage.data[0]?.attributes?.url || ""}`} alt={hero.title} width={350} height={311} /></div>
        </div> }
        </div>
    </div>
}
