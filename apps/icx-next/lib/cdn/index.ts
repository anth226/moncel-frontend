...
const queryStoryblok = async () => {
    ...
    const data: StoryblokStoryQueryResponse = await (await fetch(`${STORYBLOK_API_URL}/stories/?${filterExpression}&${tokenExpression}`)).json();
    if(data.stories.length < 1) throw Error('Storyblok story query did not match any stories'); // Error out to stop build
    return data.stories;
};
